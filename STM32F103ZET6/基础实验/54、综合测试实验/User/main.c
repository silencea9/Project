...
REINIT: /* 重新初始化 */
    ...
    /* CH395Q检测 */
    lcd_show_string(5, ypos + fsize * j, lcddev.width, lcddev.height, fsize, "CH395Q Check...", g_point_color);

    if (ch395q_init(0))system_error_show(5, ypos + fsize * (j + 1), "CH395Q Error!", fsize); /* CH395Q检测 */
    else lcd_show_string(5 + okoffset, ypos + fsize * j++, lcddev.width, lcddev.height, fsize, "OK", g_point_color);
    ...
    /* VS1053检测 */
    lcd_show_string(5, ypos + fsize * j, lcddev.width, lcddev.height, fsize, "VS1053 Check...", g_point_color);
    vs10xx_init();      /* 初始化VS1053接口 */
    vs10xx_sine_test(); /* 正弦测试 */

    if (vs10xx_ram_test() != 0X83FF)system_error_show(5, ypos + fsize * (j + 1), "VS1053 Error!", fsize);
    else lcd_show_string(5 + okoffset, ypos + fsize * j++, lcddev.width, lcddev.height, fsize, "OK", g_point_color);
