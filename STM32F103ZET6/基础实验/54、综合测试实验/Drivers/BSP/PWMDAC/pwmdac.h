...
#define PWMDAC_GPIO_PIN                     SYS_GPIO_PIN8
#define PWMDAC_GPIO_CLK_ENABLE()            do{ RCC->APB1ENR |= 1 << 2; }while(0)   /* PA口时钟使能 */

#define PWMDAC_TIMX                         TIM1
#define PWMDAC_TIMX_CHY                     1                           /* 通道Y,  1<= Y <=4 */
#define PWMDAC_TIMX_CCRX                    PWMDAC_TIMX->CCR1           /* 通道Y的输出比较寄存器 */
#define PWMDAC_TIMX_CLK_ENABLE()            do{ RCC->APB2ENR |= 1 << 11; }while(0)  /* TIM1 时钟使能 */
