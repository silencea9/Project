 ...
 * 第一次发布
 * V1.1 20221029
 * 添加vs10xx_read_para和vs10xx_save_para函数
 *
 ****************************************************************************************************
 */
...
#define VS10XX_RST_GPIO_PIN             SYS_GPIO_PIN6
#define VS10XX_RST_GPIO_CLK_ENABLE()    do{ RCC->APB2ENR |= 1 << 6; }while(0)   /* PE口时钟使能 */

#define VS10XX_XCS_GPIO_PORT            GPIOF
#define VS10XX_XCS_GPIO_PIN             SYS_GPIO_PIN7
#define VS10XX_XCS_GPIO_CLK_ENABLE()    do{ RCC->APB2ENR |= 1 << 7; }while(0)   /* PF口时钟使能 */

#define VS10XX_XDCS_GPIO_PORT           GPIOF
#define VS10XX_XDCS_GPIO_PIN            SYS_GPIO_PIN6
#define VS10XX_XDCS_GPIO_CLK_ENABLE()   do{ RCC->APB2ENR |= 1 << 7; }while(0)   /* PF口时钟使能 */

#define VS10XX_DQ_GPIO_PORT             GPIOC
#define VS10XX_DQ_GPIO_PIN              SYS_GPIO_PIN13
#define VS10XX_DQ_GPIO_CLK_ENABLE()     do{ RCC->APB2ENR |= 1 << 4; }while(0)   /* PC口时钟使能 */

/******************************************************************************************/

/* VS10XX 控制信号 */
#define VS10XX_RST(x)       sys_gpio_pin_set(VS10XX_RST_GPIO_PORT, VS10XX_RST_GPIO_PIN, x)
#define VS10XX_XCS(x)       sys_gpio_pin_set(VS10XX_XCS_GPIO_PORT, VS10XX_XCS_GPIO_PIN, x)
#define VS10XX_XDCS(x)      sys_gpio_pin_set(VS10XX_XDCS_GPIO_PORT, VS10XX_XDCS_GPIO_PIN, x)
#define VS10XX_DQ           sys_gpio_pin_get(VS10XX_DQ_GPIO_PORT, VS10XX_DQ_GPIO_PIN)
