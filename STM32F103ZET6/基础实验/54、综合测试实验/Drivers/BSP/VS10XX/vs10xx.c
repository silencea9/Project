 ...
 * V1.0 20200429
 * 第一次发布
 * V1.1 20221029(针对综合实验单独适配)
 * 1, 添加vs10xx_read_para和vs10xx_save_para函数
 * 2, 添加
 *
 ****************************************************************************************************
 */
...
/**
 * @brief       读取VS1053数据
 *  note        VS1053数据保存在:SYSTEM_PARA_SAVE_BASE+sizeof(_system_setings)之后
 * @param       vs10xxdev       : vs1053数据结构体
 * @retval      无
 */
void vs10xx_read_para(_vs10xx_obj *vs10xxdev)
{
    at24cxx_read(SYSTEM_PARA_SAVE_BASE + sizeof(_system_setings), (uint8_t *)vs10xxdev, sizeof(_vs10xx_obj));
}
...
void vs10xx_save_para(_vs10xx_obj *vs10xxdev)
{
    ...
    at24cxx_write(SYSTEM_PARA_SAVE_BASE + sizeof(_system_setings), (uint8_t *)vs10xxdev, sizeof(_vs10xx_obj));
...
{
    spi1_set_speed(SPI_SPEED_32 + nes_spped_para);  /* 设置到低速模式 */
}
...
{
    spi1_set_speed(SPI_SPEED_8 + nes_spped_para);   /* 设置到高速模式 */
}
...
void vs10xx_init(void)
{
    ...
    sys_gpio_set(VS10XX_RST_GPIO_PORT, VS10XX_RST_GPIO_PIN,
                 SYS_GPIO_MODE_OUT, SYS_GPIO_OTYPE_PP, SYS_GPIO_SPEED_HIGH, SYS_GPIO_PUPD_PU);  /* RST  引脚模式设置 */

    sys_gpio_set(VS10XX_XCS_GPIO_PORT, VS10XX_XCS_GPIO_PIN,
                 SYS_GPIO_MODE_OUT, SYS_GPIO_OTYPE_PP, SYS_GPIO_SPEED_HIGH, SYS_GPIO_PUPD_PU);  /* XCS  引脚模式设置 */

    sys_gpio_set(VS10XX_XDCS_GPIO_PORT, VS10XX_XDCS_GPIO_PIN,
                 SYS_GPIO_MODE_OUT, SYS_GPIO_OTYPE_PP, SYS_GPIO_SPEED_HIGH, SYS_GPIO_PUPD_PU);  /* XDCS 引脚模式设置 */

    sys_gpio_set(VS10XX_DQ_GPIO_PORT, VS10XX_DQ_GPIO_PIN,
                 SYS_GPIO_MODE_IN, SYS_GPIO_OTYPE_PP, SYS_GPIO_SPEED_HIGH, SYS_GPIO_PUPD_PU);   /* DQ   引脚模式设置 */
