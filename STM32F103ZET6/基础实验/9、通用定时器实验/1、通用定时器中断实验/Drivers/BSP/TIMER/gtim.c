...
void gtim_timx_int_init(uint16_t arr, uint16_t psc)
{
    GTIM_TIMX_INT_CLK_ENABLE();              // ① 时钟源
    GTIM_TIMX_INT->ARR = arr;                // ③ 自动重载寄存器 ARR
    GTIM_TIMX_INT->PSC = psc;                // ③ 预分频器 PSC
    GTIM_TIMX_INT->DIER |= 1 << 0;           // ③ 更新事件中断使能
    GTIM_TIMX_INT->CR1 |= 1 << 0;            // ③ 计数器使能
    sys_nvic_init(1, 3, GTIM_TIMX_INT_IRQn, 2); // NVIC 配置
}
